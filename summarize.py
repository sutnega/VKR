...
Поддерживает три провайдера (переключение через --provider):
  ...
  • gigachat — GigaChat от Сбера (бесплатно, 1 млн токенов/мес., нативный русский)
...
Быстрый старт:
    ...
    # GigaChat (бесплатно, лучший русский язык)
    export GIGACHAT_CREDENTIALS="Base64(ClientID:ClientSecret)"
    python summarize.py --provider gigachat
...
PROVIDER_DEFAULTS: Dict[str, Dict[str, Any]] = {
    ...
    "gigachat": {
        ...
        "description": "GigaChat (Сбер) — бесплатно, 1 млн токенов/мес., нативный русский язык",
    },
    "ollama": {
        "model": "llama3",
...
def _get_gigachat_token(credentials: str) -> str:
    """
    Получает OAuth-токен GigaChat.
    credentials — строка Base64(ClientID:ClientSecret), берётся из личного кабинета Сбера.
    Токен живёт 30 минут; для простоты получаем новый перед каждым запросом.
    ...
    # GigaChat использует самоподписанный сертификат Сбера — отключаем проверку
    import ssl
    ...
    try:
        ...
        body = e.read().decode("utf-8", errors="ignore")
        raise RuntimeError(f"GigaChat OAuth HTTP {e.code}: {body[:200]}")
    except urllib.error.URLError as e:
        raise RuntimeError(f"GigaChat OAuth сетевая ошибка: {e}")


def call_gigachat(title: str, summary: Optional[str], credentials: str, model: str) -> str:
    """
    GigaChat API от Сбера.
    Нативно поддерживает русский язык, бесплатный тариф: 1 млн токенов/мес.

    Как получить credentials:
    1. Зарегистрируйся на https://developers.sber.ru/portal/products/gigachat-api
    2. Создай проект → скопируй Client ID и Client Secret
    3. Закодируй в Base64: echo -n "ClientID:ClientSecret" | base64
    4. export GIGACHAT_CREDENTIALS="полученная_строка"
    ...
    token = _get_gigachat_token(credentials)
    ...
    # Сбер использует собственный CA — отключаем проверку сертификата
    ctx = ssl.create_default_context()
    ctx.check_hostname = False
    ctx.verify_mode = ssl.CERT_NONE
    ...
    try:
        ...
        body = e.read().decode("utf-8", errors="ignore")
        raise RuntimeError(f"GigaChat HTTP {e.code}: {body[:300]}")
    except urllib.error.URLError as e:
        raise RuntimeError(f"GigaChat сетевая ошибка: {e}")
...
def call_provider(
    ...
    try:
        if provider == "gemini":
            ...
            return call_gigachat(title, summary, api_key, model)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Резюмирование новостей через LLM (Gemini / Groq / GigaChat / Ollama)",
